<template>
	<view class="check">
		<div class="text-center">
		  <img @click="iiiii" :src="currentImage" class="rounded" alt="...">
		</div>
		<view class="detail">
...
<script>
	export default {
		...
		onLoad(o) {
			// console.log(o.id)
			console.log(this.girlList[o.id])
			this.currentImage = this.girlList[o.id]
		},
		methods:{
			iiiii() {
				// console.log("affsaf")
						...
						success:(dat) => {
							...
							if (dat.tapIndex == 0) {
								uni.getStorage({
									key:"ImageCollection",
									success:(res) => {
										if(res.data.indexOf(this.currentImage) != -1) {
											console.log("已存在于收藏夹！")
										} else {
											formerInfo = res.data
											console.log(formerInfo)
											formerInfo.push(this.currentImage)
											console.log(formerInfo)
											uni.setStorage({
												key:"ImageCollection",
												data:formerInfo,
									...
									fail: (err) => {
										console.log("no data and get the fuck away! you son of a bitch!!!!!")
										console.log(formerInfo)
										formerInfo.push(this.currentImage)
										console.log(formerInfo)
										uni.setStorage({
